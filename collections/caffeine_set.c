...
static uint64_t cff_set_get_capacity(cff_set_s *container) {
  return container->capacity;
}

static uint64_t cff_set_get_count(cff_set_s *container) {
  return container->count;
}

static cff_size cff_set_get_data_size(cff_set_s *container) {
  return container->data_size;
}

static bool cff_set_get_item_ref(cff_set_s *container, uint64_t index,
                                 uintptr_t *out) {
...
bool cff_set_add(cff_set_s *set, uintptr_t value, cff_allocator_t allocator) {
  ...
  if ((float)set->count / set->capacity >= 0.75f) {
    set = _hashmap_resize(set, set->capacity * 2, allocator);
...
bool cff_set_remove(cff_set_s *set, uintptr_t value,
  ...
  uintptr_t index_data = (uintptr_t)(set->data) + set->data_size * entry_index;
