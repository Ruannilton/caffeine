...
static inline cff_err_e cff_vector_resize(cff_vector_s* vector, uint64_t new_capacity, cff_allocator_t allocator) {

	try(cff_container_resize((cff_container_s*)(vector), new_capacity, allocator)) {
		if (new_capacity < vector->count) vector->count = new_capacity;
	}

	return CFF_ERR_NONE;
}

static inline cff_err_e cff_vector_grow(cff_vector_s* vector, cff_allocator_t allocator) {
	uint64_t new_capacity = (uint64_t)(((double)vector->count) * GROW_TAX);

	if (vector->count + 1 == vector->capacity) {
		try(cff_vector_resize(vector, new_capacity, allocator)) {}
	}
	return CFF_ERR_NONE;
}

static inline cff_err_e cff_vector_shrink(cff_vector_s* vector, cff_allocator_t allocator) {
	if (vector->count <= vector->capacity / 2) {
		try(cff_vector_resize(vector, vector->count, allocator)) {}
	}
	return CFF_ERR_NONE;
}

cff_vector_s cff_vector_create(cff_size data_size, uint64_t capacity, cff_allocator_t allocator) {
	cff_container_s container = cff_container_create(data_size, capacity, allocator);
...
cff_vector_s cff_vector_copy(cff_vector_s vector, cff_allocator_t allocator) {
	cff_container_s container = cff_container_copy(TO_CONTAINER(vector), allocator);
...
cff_err_e cff_vector_get(cff_vector_s vector, uint64_t index, uintptr_t out) {
	if (index >= vector.count) return CFF_ERR_OUT_OF_BOUNDS;
	return cff_container_get(TO_CONTAINER(vector), index, out);
}

cff_err_e cff_vector_get_ref(cff_vector_s vector, uint64_t index, uintptr_t* out) {
	if (index >= vector.count) return CFF_ERR_OUT_OF_BOUNDS;
...
cff_err_e cff_vector_insert(cff_vector_s* vector, uint64_t index, uintptr_t in, cff_allocator_t allocator) {
	if (index > vector->count) return CFF_ERR_OUT_OF_BOUNDS;

	try(cff_vector_grow(vector, allocator)) {}

	try(cff_container_insert(*(cff_container_s*)(vector), index, in)) {
		vector->count++;
	}
...
cff_err_e cff_vector_remove(cff_vector_s* vector, uint64_t index, cff_allocator_t allocator) {
	if (index >= vector->count) return CFF_ERR_OUT_OF_BOUNDS;

	try(cff_container_remove(*(cff_container_s*)(vector), index)) {
		vector->count--;
	}

	try(cff_vector_shrink(vector, allocator)) {}
...
cff_err_e cff_vector_sort(cff_vector_s vector, uint64_t start, uint64_t end, cff_order_function order) {
	return cff_container_sort(TO_CONTAINER(vector), start, end, order);
}

cff_err_e cff_vector_reserve(cff_vector_s* vector, uint64_t capacity, cff_allocator_t allocator) {
...
cff_err_e cff_vector_push_back(cff_vector_s* vector, uintptr_t in, cff_allocator_t allocator) {

	try(cff_vector_grow(vector, allocator)) {}

	try(cff_container_set(TO_CONTAINER_PTR(vector), vector->count, in)) {
		vector->count++;
	}
...
cff_err_e cff_vector_push_front(cff_vector_s* vector, uintptr_t in, cff_allocator_t allocator) {

	try(cff_vector_grow(vector, allocator)) {}

	try(cff_container_insert(TO_CONTAINER_PTR(vector), 0, in)) {
		vector->count++;
	}
...
cff_err_e cff_vector_pop_front(cff_vector_s* vector, cff_allocator_t allocator) {
	...
	if (vector->count > 0) {
		...
		try(cff_container_remove(TO_CONTAINER_PTR(vector), 0)) {
			vector->count--;
		}
