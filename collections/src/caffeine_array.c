...
cff_array_s cff_array_create(cff_size data_size, uint64_t capacity, cff_allocator_t allocator) {
...
cff_array_s cff_array_copy(cff_array_s array, cff_allocator_t allocator) {
	cff_container_s container = cff_container_copy(TO_CONTAINER(array), allocator);
	cff_array_s out_array = TO_ARRAY(container);
	return out_array;
}

cff_err_e cff_array_get(cff_array_s array, uint64_t index, uintptr_t out) {
	if (index >= array.capacity) return CFF_ERR_OUT_OF_BOUNDS;
...
cff_err_e cff_array_insert(cff_array_s* array, uint64_t index, uintptr_t in, cff_allocator_t allocator) {
	if (index > array->capacity) return CFF_ERR_OUT_OF_BOUNDS;

	try(cff_container_insert(*(cff_container_s*)(array), index, in)) {
...
cff_err_e cff_array_remove(cff_array_s* array, uint64_t index, cff_allocator_t allocator) {
	if (index >= array->capacity) return CFF_ERR_OUT_OF_BOUNDS;

	try(cff_container_remove(*(cff_container_s*)(array), index)) {
...
cff_err_e cff_array_sort(cff_array_s array, uint64_t start, uint64_t end, cff_order_function order) {
