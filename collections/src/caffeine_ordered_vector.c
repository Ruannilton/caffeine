...
static inline cff_err_e cff_ord_vector_resize(cff_ord_vector_s *vector, uint64_t new_capacity, cff_allocator_t allocator)
{

	try(cff_ord_container_resize((cff_ord_container_s *)(vector), new_capacity, allocator))
	{
...
static inline cff_err_e cff_ord_vector_grow(cff_ord_vector_s *vector, cff_allocator_t allocator)
{
	uint64_t new_capacity = (uint64_t)(((double)vector->count) * GROW_TAX);

	if (vector->count + 1 == vector->capacity)
	{
		try(cff_ord_vector_resize(vector, new_capacity, allocator)) {}
	}
	return CFF_ERR_NONE;
}

static inline cff_err_e cff_ord_vector_shrink(cff_ord_vector_s *vector, cff_allocator_t allocator)
{
	if (vector->count <= vector->capacity / 2)
	{
		try(cff_ord_vector_resize(vector, vector->count, allocator)) {}
	}
	return CFF_ERR_NONE;
}
...
{
	cff_ord_container_s container = cff_ord_container_create(data_size, capacity, comparer_fn, allocator);
...
{
	cff_ord_container_s container = cff_ord_container_copy(TO_CONTAINER(vector), allocator);
...
cff_err_e cff_ord_vector_remove(cff_ord_vector_s *vector, uint64_t index, cff_allocator_t allocator)
{
	...
	try(cff_ord_container_remove(*(cff_ord_container_s *)(vector), index))
	{
		vector->count--;
	}

	try(cff_ord_vector_shrink(vector, allocator)) {}
