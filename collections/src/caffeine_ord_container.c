...
static bool cff_ord_container_get_item_ref(cff_container_s *container, uint64_t index, uintptr_t *out)
{
	return cff_ord_container_get_ref(*(cff_ord_container_s *)container, index, out) == CFF_ERR_NONE;
}

static bool cff_ordered_container_find(cff_ord_container_s *container, uintptr_t ptr_in, uint64_t *out_index, cff_order_function comparer_function, uint64_t lenght)
{

	if (cff_binary_search(container->buffer, ptr_in, 0, lenght, container->data_size, comparer_function, out_index))
	{
		return 1;
	}
	return 0;
...
static inline uint64_t cff_ordered_container_get_index(cff_ord_container_s *container, uintptr_t value_ptr, uint64_t count)
{
	if (count == 0)
	{
		return 0;
	...
	if (count == 1)
	{
		...
		if (container->comparer(value_ptr, value, container->data_size) == CFF_GREATER)
		{
			return 1;
		}
		return 0;
	...
	uint64_t idx = 0;
	if (cff_ordered_container_find(container, value_ptr, &idx, container->comparer, count))
	{
		return idx++;
	}
...
cff_ord_container_s cff_ord_container_create(cff_size data_size, uint64_t capacity, cff_ord_e (*comparer_fn)(uintptr_t a, uintptr_t b, cff_size data_size), cff_allocator_t allocator)
{
...
cff_err_e cff_ord_container_get(cff_ord_container_s container, uint64_t index, uintptr_t out)
{
	debug_assert(out != 0);
...
cff_err_e cff_ord_container_get_ref(cff_ord_container_s container, uint64_t index, uintptr_t *out)
{
...
cff_err_e cff_ord_container_add(cff_ord_container_s container, uintptr_t in, uint64_t *out_index)
{
	debug_assert(in != 0);

	uint64_t index = cff_ordered_container_get_index(&container, in, container.capacity);
...
cff_err_e cff_ord_container_remove(cff_ord_container_s container, uint64_t index)
{
	if (index >= container.capacity)
		return CFF_ERR_OUT_OF_BOUNDS;
...
cff_err_e cff_ord_container_destroy(cff_ord_container_s container, cff_allocator_t allocator)
{
...
cff_err_e cff_ord_container_resize(cff_ord_container_s *container, uint64_t new_capacity, cff_allocator_t allocator)
{
...
cff_ord_container_s cff_ord_container_copy(cff_ord_container_s container, cff_allocator_t allocator)
{
	cff_ord_container_s new_container = cff_ord_container_create(container.data_size, container.capacity, container.comparer, allocator);
...
bool cff_ord_container_equals(cff_ord_container_s container, cff_ord_container_s other)
{
	cff_size size = (cff_size)(container.capacity * container.data_size);
...
void cff_ord_container_fill(cff_ord_container_s container, uintptr_t value, cff_size value_size)
{
...
bool cff_ord_container_find_index(cff_ord_container_s container, uintptr_t in, uint64_t *out_index)
{
	return cff_ordered_container_find(&container, in, out_index, container.comparer, container.capacity);
}