...
#define alloc_gen_array(PTR, CAPACITY) PTR = (__typeof__(PTR[0]) *)CFF_ARR_NEW(__typeof__(PTR[0]), CAPACITY, "ARRAY BLOCK")
#endif
...
#define cff_arr_impl(ARRAY_NAME, TYPE)                                                                     \
    ...
    {                                                                                                      \
        for (uint32_t i = 0; i < arr->count; i++)                                                          \
            if (CFF_CMP(arr->buffer + i, &value, sizeof(TYPE)))                                            \
                return 1;                                                                                  \
        return 0;                                                                                          \
    }                                                                                                      \
    void ARRAY_NAME##_zero(const ARRAY_NAME *arr) { CFF_ZERO(arr->buffer, sizeof(TYPE) * arr->capacity); } \
    void ARRAY_NAME##_remove(ARRAY_NAME *arr, TYPE value)                                                  \
    {                                                                                                      \
        ...
        for (uint32_t i = idx; i < arr->count; i++)                                                        \
