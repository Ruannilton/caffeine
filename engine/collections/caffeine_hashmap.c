...
static uint64_t cff_hashmp_get_capacity(void *ptr) {
  cff_hashmap_s *container = (cff_hashmap_s *)ptr;
  return container->capacity;
}

static cff_size cff_hashmp_get_data_size(void *ptr) {
  cff_hashmap_s *container = (cff_hashmap_s *)ptr;
  return container->data_size;
}

static bool cff_hashmp_get_item_ref(void *ptr, uint64_t index, uintptr_t *out) {
  cff_hashmap_s *container = (cff_hashmap_s *)ptr;
...
static cff_hashmap_s _hashmap_resize(cff_hashmap_s hashmap, uint64_t new_size,
  ...
  for (uint64_t i = 0; i < hashmap.capacity; i++) {
    ...
    uintptr_t value =
        (uintptr_t)hashmap.data * (uintptr_t)(hashmap.data_size * i);
    ...
    while (used_buffer[entry_index]) {
      collision_count++;
      entry_index =
          hashmap.hash_fn(key, hashmap.key_size, collision_count) % new_size;
    }
    ...
    uintptr_t current_data_slot =
        ((uintptr_t)data_buffer) + (uintptr_t)(hashmap.data_size * entry_index);
    cff_mem_copy((const void *)value, (void *)current_data_slot,
                 hashmap.data_size);

    uintptr_t current_key_slot =
        ((uintptr_t)key_buffer) + (uintptr_t)(hashmap.key_size * entry_index);
    cff_mem_copy((const void *)value, (void *)current_key_slot,
                 hashmap.key_size);
  }
...
cff_hashmap_s cff_hashmap_create(cff_size key_size, cff_size data_size,
  ...
  bool cnt = false;
  cff_mem_set((const void *)&cnt, (void *)used_buffer, (cff_size)sizeof(bool),
              (cff_size)(capacity * sizeof(bool)));
...
bool cff_hashmap_add(cff_hashmap_s hashmap, uintptr_t key, uintptr_t value,
  ...
  while (hashmap.used_slot[entry_index]) {
    uintptr_t current_key = (uintptr_t)hashmap.keys +
                            (uintptr_t)(collision_count * hashmap.key_size);

    if (hashmap.key_cmp_fn(key, current_key, hashmap.key_size))
      continue;

    collision_count++;
    entry_index = hashmap.hash_fn(key, hashmap.key_size, collision_count) %
                  hashmap.capacity;
  }
  ...
  uintptr_t current_data_slot =
      ((uintptr_t)hashmap.data) + (uintptr_t)(hashmap.data_size * entry_index);
  cff_mem_copy((const void *)value, (void *)current_data_slot,
               hashmap.data_size);

  uintptr_t current_key_slot =
      ((uintptr_t)hashmap.keys) + (uintptr_t)(hashmap.key_size * entry_index);
...
bool cff_hashmap_get(cff_hashmap_s hashmap, uintptr_t key, uintptr_t *value) {
  ...
  uintptr_t current_key =
      (uintptr_t)hashmap.keys + (uintptr_t)(collision_count * hashmap.key_size);

  while (!hashmap.used_slot[entry_index] &&
         !hashmap.key_cmp_fn(key, current_key, hashmap.key_size)) {
    current_key = (uintptr_t)hashmap.keys +
                  (uintptr_t)(collision_count * hashmap.key_size);
    collision_count++;
  ...
  if (value != NULL) {
    *value = ((uintptr_t)hashmap.data) +
             (uintptr_t)(hashmap.data_size * entry_index);
    return true;
  }
...
void cff_hashmap_clear(cff_hashmap_s hashmap) {
  ...
  bool cnt = false;
  cff_mem_set(&cnt, hashmap.used_slot, (cff_size)sizeof(bool),
...
cff_hashmap_s cff_hashmap_copy(cff_hashmap_s hashmap, cff_allocator allocator) {
  ...
  cff_hashmap_s new_hash = cff_hashmap_create(
      hashmap.key_size, hashmap.data_size, new_size, hashmap.key_cmp_fn,
...
cff_hashmap_s cff_hashmap_clone(cff_hashmap_s hashmap,
  ...
  cff_mem_copy(hashmap.data, new_hash.data,
               (cff_size)(hashmap.data_size * hashmap.capacity));
  cff_mem_copy(hashmap.keys, new_hash.keys,
               (cff_size)(hashmap.key_size * hashmap.capacity));
  cff_mem_copy(hashmap.used_slot, new_hash.used_slot,
               (cff_size)(sizeof(bool) * hashmap.capacity));
