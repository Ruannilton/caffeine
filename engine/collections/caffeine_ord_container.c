...
static uint64_t cff_ord_container_get_capacity(void *ptr) {
  cff_container_s *container = (cff_container_s *)ptr;
  return container->capacity;
}

static cff_size cff_ord_container_get_data_size(void *ptr) {
  cff_container_s *container = (cff_container_s *)ptr;
  return container->data_size;
}

static bool cff_ord_container_get_item_ref(void *container, uint64_t index,
                                           uintptr_t *out) {
  return cff_ord_container_get_ref(*(cff_ord_container_s *)container, index,
                                   out) == CFF_ERR_NONE;
}

static bool cff_ordered_container_find(void *ptr, uintptr_t ptr_in,
                                       uint64_t *out_index,
                                       cff_order_function comparer_function,
                                       uint64_t lenght) {
  cff_ord_container_s *container = (cff_ord_container_s *)ptr;
...
static inline uint64_t cff_ordered_container_get_index(void *ptr,
                                                       uintptr_t value_ptr,
                                                       uint64_t count) {
  cff_ord_container_s *container = (cff_ord_container_s *)ptr;
  if (count == 0) {
